...
/*   Updated: 2022/06/12 00:57:03 by sarchoi          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(Form const & f) {
	if (!f.getIsSigned()) {
		std::cout << this->getName() << " couldn't execute " << f.getName() << " because it is not signed" << std::endl;
		return;
	}
	if (f.execute(*this))
		std::cout << this->getName() << " executed " << f.getName() << std::endl;
	else
		std::cout << this->getName() << " couldn't execute " << f.getName() << " because " << this->getName() << " is not high enough grade" << std::endl;
}
